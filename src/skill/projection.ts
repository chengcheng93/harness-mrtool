...
export function renderSkillInstructions(options: SkillProjectionOptions): string {
  ...
  return [
    ...
    "Show the title, body, target branch, and push plan. After confirmation, rerun with --push. The user creates the MR in GitLab and selects live labels and reviewers. The fixed pool has no scheduling labels. Ordinary handoff is not verified MR creation; never use SSH push options to bypass mandatory API label verification.",
    ...
    "",
    "Mandatory labels are selected by the CLI from the committed diff: one type, p2 by default, and lifecycle status. Week labels are retired. SSH-only MR creation is blocked.",
