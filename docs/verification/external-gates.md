...
| Immutable GitHub release assets | Completed for CLI 0.1.0 | Release `cli-v0.1.0` is immutable. The workflow verified final bytes and downloaded draft assets before publishing; the Release page exposes the generated GitHub Release attestation. The repository is now public; the release was built while it was private, so Artifact Attestations were skipped for this version. |
