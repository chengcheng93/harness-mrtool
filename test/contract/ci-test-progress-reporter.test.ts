...
test('native CI reporter finds nested codes without reflecting unknown error metadata',async()=>{
 ...
 assert.equal(await failure({details:{actual:'application-bundle:version-exit-7'}}),failPrefix+' diagnostic=application-bundle:version-exit-7\n');
 ...
 assert.equal(await failure({diagnostic:'windows-lock-helper:compile'}),failPrefix+' diagnostic=windows-lock-helper:compile\n');
 assert.equal(await failure({diagnostic:'SECRET-DIAGNOSTIC'}),failPrefix+'\n');
