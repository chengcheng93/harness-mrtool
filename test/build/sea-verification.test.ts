...
const expectedStdout = expectedSeaSelfTestStdout("0.1.6");
...
test("pins the embedded request and output contract probe result", () => {
  assert.equal(expectedSeaContractProbeStdout("0.1.6"), expectedProbeStdout);
  assert.equal(expectedSeaRendererProbeStdout("0.1.6"), expectedRendererProbeStdout);
});
