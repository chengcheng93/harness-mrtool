...
function failureDetails(data) {
  ...
  for (let depth = 0; depth < MAX_ERROR_DEPTH && error !== undefined; depth++) {
    ...
    if (diagnostic === undefined && typeof actual === 'string' && /^(?:windows-helper|native-store|native-readiness|managed-installation|update-state|installation-journal|application-bundle):[a-z-]{1,32}$/u.test(actual)) {
      diagnostic = actual;
    }
